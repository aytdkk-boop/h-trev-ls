...
(function() {
  ...
  // ===== ФОРМАТИРОВАНИЕ ДАТЫ =====
  // Сервер возвращает UTC без суффикса Z. Добавляем 'Z', чтобы JS понял, что это UTC,
  // затем переводим в локальное время устройства.
  function formatDateTime(isoStr) {
    try {
      let s = String(isoStr).trim();

      // Если строка уже ISO-совместима (с T и таймзоной) — используем как есть
      if (s.includes('T') && (s.endsWith('Z') || /[+-]\d{2}:?\d{2}$/.test(s))) {
        const dt = new Date(s);
        if (!isNaN(dt.getTime())) return formatLocal(dt);
      }

      // Иначе считаем, что это UTC без суффикса
  ...
  confirmYesBtn.addEventListener('click', function() {
    ...
    // Плавно скрываем контейнер с ключом
    setTimeout(function() {
      hideKeyCard();
    }, 100);
