...
(function() {
  ...
  geoAllowBtn.addEventListener('click', async function() {
    ...
    if (!isSecure) {
      geoStatus.textContent = 'Нужен HTTPS для запроса геолокации';
      geoAllowBtn.disabled = false;
      geoCancelBtn.disabled = false;
      return;
    }

    geoStatus.textContent = 'Ожидаем разрешения...';
    ...
    const coords = await getCoordsWithRetries(60, 3000);

    if (coords.lat === null) {
      geoStatus.textContent = 'GPS недоступен, определяем по сети...';
      const ipCoords = await getCoordsByIP();

      if (ipCoords.lat === null) {
        geoStatus.textContent = 'Не удалось определить. Включите GPS.';
        currentLocation = {
          address: 'Включите геолокацию на устройстве',
          coords: '',
          lat: null,
          lon: null,
          resolved: true
        };
        showLocationOnPage('Включите геолокацию на устройстве', '');
        showGeoInstructions();
        geoAllowBtn.disabled = false;
        geoCancelBtn.disabled = false;
        return;
  ...
  function showGeoInstructions() {
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
    const isAndroid = /Android/.test(navigator.userAgent);
    let hint = 'Включите геолокацию в настройках устройства.';
    if (isIOS) hint = 'iPhone: Настройки → Конфиденциальность → Службы геолокации → Вкл.';
    else if (isAndroid) hint = 'Android: Настройки → Местоположение → Вкл.';
    geoStatus.textContent = hint;
  }
