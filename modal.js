...
(function() {
  ...
  async function bootWithDelay() {
    // Быстрая проверка: если в localStorage есть флаг активации,
    // прячем окно сразу, чтобы не было мелькания
    let localActivated = false;
    try {
      localActivated = localStorage.getItem('hitrevil_activated') === 'true';
    } catch (e) {}
    ...
    if (isActive) {
      ...
      // Сбрасываем состояние окна — кнопка и поле пустые
      activateBtn.classList.remove('visible');
      keyInput.value = '';
