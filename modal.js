...
(function() {
  ...
  // Параллельно: проверяем ключ + держим экран 8 секунд.
  // Если ключ активен — показываем главную страницу.
  // Если ключа нет / сброшен / истёк — показываем окно активации.

  async function bootWithDelay() {
    // Запускаем проверку сразу, но она не блокирует
    const activationPromise = checkActivationOnStart();

    // Ждём одновременно проверку и 8 секунд
    ...
    if (isActive) {
      // Ключ активен — плавно показываем страницу
      mainContent.classList.add('active');
      if (window.onSiteActivated) window.onSiteActivated();
    } else {
      // Ключа нет — показываем окно активации
      modalOverlay.classList.remove('hidden');
      modal.classList.remove('fade-out');
    }
  }

  // Запускаем через небольшую задержку, чтобы DOM успел прогрузиться
